...
if (isset($_POST["send"])) {
    ...
    $contenue = htmlspecialchars(stripslashes(trim($_POST["contenue"])));
    ...
    <header class="flex justify-between items-center lg:pl-32 pl-4 lg:pr-32 pr-4 h-24 border-b-[1px] border-[#ed231a]">
        ...
        <nav class="flex gap-5 items-center">
            <form action="index.php">
                <button type="submit" id="acceuil" class="bg-white border-[1px] border-black px-2 py-1 rounded-lg hover:text-white text-xl" style="box-shadow: 8px 8px 5px #7e1d50;">Accueil</button>
            </form>
            <?php if (isset($_SESSION["user"])) {
                if ($_SESSION["user"]["statut"] == "admin") { ?>
                    <form action="admin.php">
                        <button type="submit" id="admin" class="bg-white border-[1px] border-black px-2 py-1 rounded-lg hover:text-white text-xl" style="box-shadow: 8px 8px 5px #7e1d50;">Admin</button>
                    </form>
                <?php } ?>
                <form action="logout.php">
                    <button type="submit" id="deco" class="bg-white border-[1px] border-black px-2 py-1 rounded-lg hover:text-white text-xl" style="box-shadow: 8px 8px 5px #7e1d50;">Déconnexion</button>
                </form>
            <?php } ?>
        </nav>
    </header>
    <main class="flex flex-col items-center bg-cover bg-center" style="background-image: url(https://static.vecteezy.com/ti/vecteur-libre/p3/265609-illustrationle-paysage-rouge-vectoriel.png)">
        <?php foreach ($bdd->getAllUsers() as $user) {
            if ($_SESSION["user"]["id"] == $user["id"]) { ?>
                <section class="border-2 text-white border-black p-5 flex justify-between gap-5 rounded-lg w-[40vw] bg-[#7e1d50] mt-10" style="box-shadow: 8px 8px 5px white;">
        ...
        <section class="flex items-center flex-col my-8">
            ...
            <form class="flex flex-col gap-5 border-2 border-black p-5 rounded-lg w-[60vw] bg-[#7e1d50]" action="" method="post" style="box-shadow: 8px 8px 5px white;">
                <h1 class="text-2xl mb-2 text-white text-center">Création d'un Post:</h1>
                <input class="border-2 border-black p-1 rounded-lg" type="text" name="titre" id="" placeholder="titre">
                <textarea class="border-2 border-black p-1 rounded-lg overflow-y-auto resize-none h-32" name="contenue" id="" placeholder="contenue"></textarea>
                <label class="text-center text-white">Sous-catégorie associée:</label>
                <select name="sous_categorie" class="text-center border-2 border-black rounded-lg">
                ...
                <button class="border-2 border-black p-1 bg-white rounded-lg" type="submit" name="send">Ajouter</button>
            </form>
        </section>
    </main>
