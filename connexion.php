...
if (isset($_POST['connexion'])) {
    ...
    $email = htmlspecialchars(stripslashes(trim($_POST['c-email'])));
    $mdp = htmlspecialchars(stripslashes(trim($_POST['c-mdp'])));
    ...
    <a href="index.php"><svg width="50px" height="50px" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
        ...
        <nav class="flex gap-5 items-center">
            <form action="index.php" method="post">
                <button type="submit" id="acceuil" class="bg-white border-[1px] border-black px-2 py-1 rounded-lg text-xl" style="box-shadow: 8px 8px 5px #7e1d50;">Accueil</button>
            </form>
        </nav>
    </header>
    <main class="flex items-center flex-col pt-[10vh] h-[75.2vh] bg-cover bg-center" style="background-image: url(https://static.vecteezy.com/ti/vecteur-libre/p3/265609-illustrationle-paysage-rouge-vectoriel.png)">
        <form class="flex flex-col items-center gap-5 text-white border-2 border-white p-5 rounded-lg w-[30vw] bg-[#7e1d50]" action="" method="post" style="box-shadow: 8px 8px 5px white;">
            <h1 class="text-2xl mb-2">Connexion:</h1>
            <input class="border-2 border-black p-1 text-black rounded-lg placeholder-black w-full" type="email" name="c-email" id="" placeholder="Email">
            <input class="border-2 border-black p-1 text-black rounded-lg placeholder-black w-full" type="password" name="c-mdp" id="" placeholder="Mot-de-passe">
            <button class="border-2 border-black p-1 text-black bg-white rounded-lg w-[10vw]" type="submit" name="connexion">Envoyer</button>
