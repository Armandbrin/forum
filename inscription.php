...
if (isset($_POST['inscription'])) {
    if (!empty($_POST['i-nom']) && !empty($_POST['i-prenom']) && !empty($_POST['i-email']) && !empty($_POST['i-mdp'])) {

        $nom = htmlspecialchars(stripslashes(trim($_POST['i-nom'])));
        $prenom = htmlspecialchars(stripslashes(trim($_POST['i-prenom'])));
        $email = htmlspecialchars(stripslashes(trim($_POST['i-email'])));
        $mdp = htmlspecialchars(stripslashes(trim($_POST['i-mdp'])));
        ...
        $mess = "Veuillez remplir touts les champs";
    }
}
?>
<!DOCTYPE html>
...
<body>
    <header class="flex justify-between items-center lg:pl-32 pl-4 lg:pr-32 pr-4 h-24 border-b-[1px] border-[#ed231a]">
        ...
        <nav class="flex gap-5 items-center">
            <form action="index.php">
                <button type="submit" class="border-[1px] border-black px-2 py-1 rounded-lg hover:text-white text-xl" id="acceuil" style="box-shadow: 8px 8px 5px #7e1d50;">Accueil</button>
            </form>
    ...
    <main class="flex flex-col items-center bg-cover bg-center bg-fixed bg-gray-300 pt-10 pb-10 min-h-[75vh]" style="background-image: url(https://static.vecteezy.com/ti/vecteur-libre/p3/265609-illustrationle-paysage-rouge-vectoriel.png)">
        <form class="flex flex-col items-center gap-5 border-2 border-black p-5 rounded-lg w-[25vw] bg-[#7e1d50]" method="post" style="box-shadow: 8px 8px 5px white;">
            <h1 class="text-2xl text-white mb-2">Inscription:</h1>
            <input class="border-2 border-black p-1 rounded-lg placeholder-black w-full" type="text" name="i-nom" id="" placeholder="Nom">
            <input class="border-2 border-black p-1 rounded-lg placeholder-black w-full" type="text" name="i-prenom" id="" placeholder="Prénom">
            <input class="border-2 border-black p-1 rounded-lg placeholder-black w-full" type="email" name="i-email" id="" placeholder="Email">
            <input class="border-2 border-black p-1 rounded-lg placeholder-black w-full" type="password" name="i-mdp" id="" placeholder="Mot-de-passe">
            <button class="border-2 border-black p-1 bg-white rounded-lg w-full" type="submit" name="inscription">Envoyer</button>
